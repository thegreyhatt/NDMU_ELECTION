...
{{-- {{dd($candidates->where('position_id','=',array_search('President', $positions))->count())}} --}}
    <div class="container">
            ...
            <form method="POST" action="{{ url('/voting/votes/council') }}" accept-charset="UTF-8" class="form-group" enctype="multipart/form-data">
                {{ csrf_field() }}
                @section('office')
                - COUNCIL
                @endsection
            ...
            <div class="row">
                ...
                @foreach($candidates as $candidate)
                    @if($candidate->position->position == $position && $candidate->college_id == $college_id)
                        @if ($count == 6)
                            <br>
                        @endif
                        @if ($col<6 && $flag !=1)
...
function confirmation(){
    if(confirm('Do you want to submit votes for council?')){
